...
try {
    ...
    if (is_array($printers) && count($printers) > 0) {
        // Check if host already has a default printer
        $def_chk = $pdo->prepare("SELECT COUNT(*) as cnt FROM printers WHERE host_id = ? AND is_default = 1");
        $def_chk->execute([$host['id']]);
        $has_default = ((int)($def_chk->fetch()['cnt'] ?? 0)) > 0;
        ...
        foreach ($printers as $p) {
            ...
            if ($existing) {
                ...
                $ins->execute([$host['id'], $name, $sys_name, $is_virtual, $is_default]);
            }
        }
    }
